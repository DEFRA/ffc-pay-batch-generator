...
public class SITICOHTRFactory : BaseBatchFactory
{
    public SITICOHTRFactory(Request request) : base(request)
    {
    }
    ...
    public override string GetContent()
    {
        ...
        for (int i = 0; i < batchSize; i++)
        {
            ...
            sb.AppendLine(string.Format("L^ESFIO{0}^{1}^{2}^80201^DRD10^Z{3}^{4}^N^1^G00 - Gross value of claim^{5}^{5}^SOS270",
                invoiceNumber.ToString("D7"),
                invoiceValue,
                schemeYear,
                frn.ToString("D8")[^8..],
